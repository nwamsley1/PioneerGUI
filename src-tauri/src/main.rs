...
enum ConfigLoadError {
    #[error("Pioneer binary not found in PATH")]
    MissingBinary,
...
fn try_fetch_build_defaults() -> Result<Value, ConfigLoadError> {
    let pioneer = which("Pioneer").map_err(|_| ConfigLoadError::MissingBinary)?;
...
async fn run_pioneer(window: Window, request: RunRequest) -> Result<RunStartedPayload, String> {
    let pioneer_path =
        which("Pioneer").map_err(|_| "Pioneer binary not found in PATH".to_string())?;
